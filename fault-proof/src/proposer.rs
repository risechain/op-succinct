    ...
    H: OPSuccinctHost + Clone + Send + Sync + 'static,
{
            ...
            for (index, game_address) in games {
                ...
                match status {
                    GameStatus::IN_PROGRESS => {
                        ...
                        let parent_resolved = parent_index == u32::MAX ||
                            actions.iter().any(|a| {
                                matches!(
                                    a,
    ...
    pub async fn prove_game(
        ...
        let tasks = ranges.into_iter().enumerate().map(|(idx, (start, end))| {
            ...
            async move {
                ...
                let sp1_stdin = this.range_proof_stdin_with_cache(start, end).await?;
    ...
    #[tracing::instrument(name = "[[Precaching]]", skip(host, safe_db_fallback), fields(game_address = ?game_address))]
